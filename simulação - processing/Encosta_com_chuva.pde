...
// Função para desenhar gotas de chuva
void drawRain() {
...
void addHousesAroundSensor() {
  ...
  for (int i = 0; i < numHouses; i++) {
    float angle = map(i, 0, numHouses, 0, TWO_PI);
    float hx = sensorX + cos(angle) * 5;
    float hy = sensorY + sin(angle) * 5;
    houses.add(new House(hx, hy, terrain[int(hx)][int(hy)]));
  }
}
